...
# Update of Combine 5 to use parallel processing with progress bars
...
# Don't parallelise this section
error_networks <- map(error_networks, ~map(.x, ~map(.x, computeCentrality)))

# recalculate global network metrics
with_progress({
  p <- progressor(steps = length(error_networks))
  
  metrics_error_networks <- future_imap(error_networks, function(error_type_list, error_type_name) {
    p(sprintf("Processing error type: %s", error_type_name))
...
with_progress({
  ...
  centrality_bias_all <- future_imap(error_networks, function(error_type_list, error_type_name) {
    p(sprintf("Computing node bias: %s", error_type_name))
    
    imap(error_type_list, function(dataset_list, dataset_name) {
      imap(dataset_list, function(perturbed_list, error_label) {
        computeNodeBias(
